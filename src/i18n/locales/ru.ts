...
export const ru: Partial<Messages> = {
  ...
  'panel.overlay.capacityTitle': 'Кадр велик для этого кодека',
  'panel.overlay.capacityBody':
    'У {format} закончилась память на кадре такого размера. Его сборка для браузера ограничена 2 ГБ, и libjxl проходит этот предел где-то за 20 Мп.',
  'panel.overlay.capacityHint':
    'Настройки тут ничего не меняют — ни качество, ни effort. Остальные форматы этот кадр берут.',
  ...
  'panel.interpolated': 'Предпросмотр в пониженном разрешении — увеличенные пиксели здесь интерполированы, это не работа кодека',
